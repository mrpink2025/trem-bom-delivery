...
DO $$ BEGIN
    -- Add arrive_radius_m to restaurants table (as store_units doesn't exist)
    ...
    -- Add arrive_radius_client_m to orders table
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'orders' AND column_name = 'arrive_radius_client_m') THEN
        ALTER TABLE orders ADD COLUMN arrive_radius_client_mint DEFAULT 60 CHECK (arrive_radius_client_m > 0);
    END IF;
END $$;
