...
BEGIN
  ...
  IF EXISTS (
    ...
    -- Remover todas as políticas que dependem da coluna status
    ...
    DROP POLICY IF EXISTS "Couriers can accept available orders" ON public.orders;
    
    -- Criar coluna temporária com tipo ENUM
    ALTER TABLE public.orders ADD COLUMN new_status order_status;
    
    -- Mapear valores para a nova coluna
    UPDATE public.orders SET new_status = 
      CASE status
        WHEN 'pending' THEN 'placed'::order_status
        WHEN 'pending_payment' THEN 'placed'::order_status
        WHEN 'placed' THEN 'placed'::order_status
        ...
        WHEN 'canceled' THEN 'cancelled'::order_status
        ELSE 'placed'::order_status
      END;
    
    -- Remover coluna antiga e renomear nova
    ALTER TABLE public.orders DROP COLUMN status;
    ALTER TABLE public.orders RENAME COLUMN new_status TO status;
    
    -- Definir valor padrão e NOT NULL
    ...
    FOR SELECT USING (
      has_role('courier'::user_role) AND (
        (auth.uid() = courier_id) OR 
        (courier_id IS NULL AND status = ANY (ARRAY['confirmed'::order_status, 'preparing'::order_status, 'ready'::order_status]))
      )
    );
    ...
    FOR UPDATE USING (
      ...
      status = ANY (ARRAY['confirmed'::order_status, 'preparing'::order_status, 'ready'::order_status])
    ) WITH CHECK (
      has_role('courier'::user_role) AND auth.uid() = courier_id
    );
  END IF;
END $$;
...
-- 6. Trigger para validação de status
