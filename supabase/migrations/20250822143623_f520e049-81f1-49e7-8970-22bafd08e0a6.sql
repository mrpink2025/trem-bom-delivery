-- Atualizar função para expandir busca quando poucos resultados próximos
...
BEGIN
  ...
  IF nearby_count < 5 AND client_city_param IS NOT NULL THEN
    ...
    SELECT 
      ...
      r.latitude,
      r.longitude,
      true as in_same_city,  -- Todos são da mesma cidade
    ...
    WHERE r.is_active = true
      ...
      AND (NOT only_open_param OR 
           ...
           jsonb_typeof(r.opening_hours) = 'null' OR
           r.is_active = true)
    LIMIT (limit_param - nearby_count); -- Completar até o limite
  END IF;
  ...
  ORDER BY 
    -- Primeiro: restaurantes próximos da mesma cidade
    CASE WHEN in_same_city AND NOT search_expanded THEN 0 ELSE 1 END,
    -- Segundo: outros restaurantes próximos  
