-- Phase 1: Database Security Fixes (Final - Valid Operations)

-- 1. Fix existing function conflicts by dropping first
...
-- 3. Update ensure_single_default_address function  
...
-- 4. Create missing psychological rounding function for pricing security
CREATE FUNCTION public.apply_psychological_rounding(amount numeric, rounding_type text DEFAULT 'ROUND_UP_99')
RETURNS numeric
LANGUAGE plpgsql
IMMUTABLE
SET search_path = public
...
-- 5. Add security audit log using valid operation
INSERT INTO public.audit_logs (
  table_name, operation, new_values, user_id
) VALUES (
  ...
  jsonb_build_object(
    'security_fixes_applied', ARRAY[
      ...
      'created_pricing_rounding_function'
    ],
    'applied_at', now(),
    'severity', 'CRITICAL'
  ),
