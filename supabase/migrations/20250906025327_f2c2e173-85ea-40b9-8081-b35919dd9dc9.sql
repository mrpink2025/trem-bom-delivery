-- Fix create_pool_match_no_debit function to include ready field
CREATE OR REPLACE FUNCTION create_pool_match_no_debit(
  p_user_id uuid,
  p_mode text, 
  p_buy_in numeric,
...
BEGIN
  ...
  -- Check wallet balance without locking
  ...
  -- Check available balance (but don't reserve)
  ...
  -- Create match with correct schema (no credit reservation)
  INSERT INTO pool_matches(
    ...
    jsonb_build_array(
      jsonb_build_object(
        'userId', p_user_id, 
        'seat', 0, 
        'connected', false, -- Set to false initially, WebSocket will set to true
        'ready', false,     -- Add ready field set to false initially
        'mmr', 1000
      )
    ),
    jsonb_build_object(
