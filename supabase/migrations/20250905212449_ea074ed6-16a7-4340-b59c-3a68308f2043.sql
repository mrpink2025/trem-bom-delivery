...
-- Note: This table may not exist in all environments, so we'll check first
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'spatial_ref_sys') THEN
        ALTER TABLE public.spatial_ref_sys ENABLE ROW LEVEL SECURITY;
        
        -- Drop existing policy if it exists
        DROP POLICY IF EXISTS "Authenticated users can read spatial reference systems" ON public.spatial_ref_sys;
...
-- 3. Create a missing psychological rounding function for pricing
...
-- 4. Add security audit log for these changes
INSERT INTO public.audit_logs (
  table_name, operation, new_values, user_id
) VALUES (
