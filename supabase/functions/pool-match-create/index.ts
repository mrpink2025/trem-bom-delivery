...
const CORS_ALLOW_ORIGIN = Deno.env.get("CORS_ALLOW_ORIGIN") || ""; // prod: seu domínio; vazio = ecoa Origin (dev)
const EDGE_DEBUG = Deno.env.get("EDGE_DEBUG") === "true"; // se true, devolve detalhes de erro
...
function json(status: number, body: any, req: Request, requestId?: string) {
  const safeBody = EDGE_DEBUG ? body : (() => {
    // em prod ocultamos detalhes sensíveis
    const { error, fieldErrors, requestId: _, ...rest } = body || {};
    return { error, fieldErrors, requestId };
  })();
  return new Response(JSON.stringify({ requestId, ...safeBody }), {
    status,
...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method !== "POST")   return json(405, { error: "METHOD_NOT_ALLOWED" }, req, requestId);
    ...
    if (!auth?.startsWith("Bearer ")) return json(401, { error: "UNAUTHENTICATED" }, req, requestId);
    ...
    catch { return json(401, { error: "UNAUTHENTICATED" }, req, requestId); }
    ...
    if (error) {
      ...
      if (msg.includes("INSUFFICIENT_FUNDS")) return json(409, { error: "INSUFFICIENT_FUNDS", msg: EDGE_DEBUG ? msg : undefined }, req, requestId);
      if (msg.includes("WALLET_NOT_FOUND"))   return json(404, { error: "WALLET_NOT_FOUND",   msg: EDGE_DEBUG ? msg : undefined }, req, requestId);
      if (code === "23505" || msg.includes("RETRY_JOIN_CODE")) return json(503, { error: "RETRY_JOIN_CODE", msg: EDGE_DEBUG ? msg : undefined }, req, requestId);
      return json(500, { error: "INTERNAL", msg: EDGE_DEBUG ? msg : undefined }, req, requestId);
    }

    return json(201, { ...(data ?? {}) }, req, requestId);
  } catch (e) {
    console.error("[pool-match-create] CRASH", { requestId, err: String(e) });
    return json(500, { error: "INTERNAL", err: EDGE_DEBUG ? String(e) : undefined }, req, requestId);
  }
});