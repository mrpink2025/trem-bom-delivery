...
BEGIN
    ...
    INSERT INTO user_wallets (user_id, balance_cents, locked_balance_cents, created_at, updated_at)
    VALUES (target_user_id, credit_amount, 0, now(), now())
    ON CONFLICT (user_id) 
    DO UPDATE SET 
        balance_cents = user_wallets.balance_cents + credit_amount,
        updated_at = now();
    ...
    VALUES (
        ...
        credit_amount, 
        (SELECT balance_cents FROM user_wallets WHERE user_id = target_user_id),
